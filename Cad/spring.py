...
from PySide2.QtWidgets import (
    QWidget, QApplication, QSlider, QGraphicsView, QGraphicsScene, QVBoxLayout, QStyle)
    ...
    try:
        if switch == 0:
            ...
            App.Console.PrintMessage(
                "Chrono begin   : "+str(time.strftime('%X'))+"\n")
        else:
            arrivee = time.time()  # time.clock()
            App.Console.PrintMessage(
                "Chrono end     : "+str(time.strftime('%X'))+"\n")
            parcouru = ((arrivee - depart)/60.0)
            App.Console.PrintError(
                "Time execution : "+str("%.3f" % parcouru)+" min"+"\n\n")
        return parcouru
...
param = FreeCAD.ParamGet(
    "User parameter:BaseApp/Preferences/Macro")  # macro path
...
global rayon
rayon = 20.0   # radius of spring
global pas
pas = 15.0   # " ! float " (pas) pitch of spire
global precision
precision = 5.0    # " ! float " 360/precision number points for 1 turn
global typeLine
typeLine = 0      # typeLine 0=BSpline or 1=Wire
global helixS
helixS = 0.0    # tableau
global numberSpiresModified
numberSpiresModified = 1      # number (pas) pitch to modify
global pasSpire
pasSpire = 0  # pas    # (pas) pitch of spire to modify
global radiusS
radiusS = 0.0    # tableau radius to modify
global rayonSpire
rayonSpire = 0  # rayon  # new radius
global affPoint
affPoint = 0      # aff points
global debutAngle
debutAngle = 0      # begin angle rotation
global finAngle
finAngle = 360    # end angle rotation
global modifyAngle
modifyAngle = 0      # interrupteur angle
global radius_2_Cone
radius_2_Cone = rayon  # radius_2_Cone en cas d'helice
global spireConeUne
spireConeUne = 0      # interrupteur cone si une seule spire spireConeUne = 1
global spireConeComp
spireConeComp = 0      # compensation pour cone
global spireReverse
spireReverse = 0      # 0 = sens antihoraire si = 1 sens horaire
global lissageSpire
lissageSpire = 0      # niveau de lissage
global lissageS
lissageS = 0      # tableau
global fichierOpen
fichierOpen = 0      # switch fichier precision
global nomF
nomF = "Name File"  # name file
global points
points = []     # tableau points
global zoom
zoom = 0      # zoom textEdit
global ressort
ressort = ""
...
ui = ""     # resident
global sel
sel = ""     # resident selection

global pointsDirection
pointsDirection = []     # tableau direction
global Direction_Begin
Direction_Begin = 0      # direction tableau 0
global Direction_Distance
Direction_Distance = 0.1    # precision Distance coupure et deplacement
global plr
plr = ""     # placement sur selection
global coor_Z
coor_Z = 0.0    # pour calcul placement
global hauteurCylindre
hauteurCylindre = 0.0    # adapter la hauteur du ressort
global vecteurSouris
vecteurSouris = ""     # vecteur Selobserver
global centerFaceOrPoint
centerFaceOrPoint = 0      # centrage de l'axe sur face ou au point souris
global axisSpring
axisSpring = ""     # axe de l'objet pour le ressort
global switchReverse
switchReverse = 0      # switchReverse
global switchAdaptRadius
switchAdaptRadius = 0      # switchAdaptRadius
global selectedCircle
selectedCircle = []     # tableau Circle
...
##
FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                 __Title__).SetString(u"Version", __Version__ + " (" + __Date__ + ")")
##
# ; seTPositionFlyRightLeft  = 2    # 1 = fly, 2 = RightDock other= LeftDock
global seTPositionFlyRightLeft
seTPositionFlyRightLeft = FreeCAD.ParamGet(
    u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" + __Title__).GetInt(u"seTPositionFlyRightLeft")
if seTPositionFlyRightLeft == 0:
    seTPositionFlyRightLeft = 1
FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                 __Title__).SetInt(u"seTPositionFlyRightLeft", seTPositionFlyRightLeft)  # *1, 2, other
##
# ; seTWidgetPosition        = 0   # position the widget Left or Right
global seTWidgetPosition
seTWidgetPosition = FreeCAD.ParamGet(
    u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" + __Title__).GetBool(u"seTWidgetPosition")
FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                 __Title__).SetBool(u"seTWidgetPosition", seTWidgetPosition)                # True or False
##
global switchQFileDialogMint    # special LinuxMint
switchQFileDialogMint = FreeCAD.ParamGet(
    "User parameter:BaseApp/Preferences/Macros/FCMmacros/" + __Title__).GetBool("switchQFileDialogMint")
if switchQFileDialogMint == 0:
    FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                     __Title__).SetBool("switchQFileDialogMint", switchQFileDialogMint)
try:
    if os.uname()[1] == "mint":
        switchQFileDialogMint = 1
        FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                         __Title__).SetBool("switchQFileDialogMint", switchQFileDialogMint)  # *Special Mint
except Exception:
    None
##
global setPathLatestDirectory  # ; setPathLatestDirectory   = "C:\ ???"
setPathLatestDirectory = FreeCAD.ParamGet(
    u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" + __Title__).GetString(u"setPathLatestDirectory")
if setPathLatestDirectory == "":
    setPathLatestDirectory = path
FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                 __Title__).SetString(u"setPathLatestDirectory", setPathLatestDirectory)  # *"C:\ ???"
##
#### Configuration end ################################################
##
try:
...
def errorDialog(msg):
    diag = QtWidgets.QMessageBox(
        QtWidgets.QMessageBox.Critical, u"Error Message", msg)
    # PySide #cette fonction met la fenetre en avant
    diag.setWindowFlags(PySide2.QtCore.Qt.WindowStaysOnTopHint)
...
#        MainWindow.setMaximumSize(QtCore.QSize(270, 660))
        ...
        ####
        # cadre scrollarea contenant le widget GUI
        ...
        self.DS_Numb_Spires.valueChanged.connect(
            self.on_DS_Numb_Spires)  # connection doubleSpinBox
...
#        self.DS_Pas_Spring.setMinimum(0.001)
        ...
        self.DS_Pas_Spring.valueChanged.connect(
            self.on_DS_Pas_Spring)  # connection doubleSpinBox
...
#        self.DS_Radius_Sping.setMinimum(0.001)
        ...
        self.DS_Radius_Sping.valueChanged.connect(
            self.on_DS_Radius_Sping)  # connection doubleSpinBox
        ...
        self.DS_Precision_Turn.valueChanged.connect(
            self.on_DS_Precision_Turn)  # connection doubleSpinBox
...
#        self.DS_Radius_2_Cone.setMinimum(rayon)
        ...
        self.DS_Radius_2_Cone.valueChanged.connect(
            self.on_DS_Radius_2_Cone)   # connection

        self.CH_Cone = QtWidgets.QCheckBox()
        self.CH_Cone.setEnabled(True)
        self.CH_Cone.clicked.connect(
            self.on_CH_Cone)                          # connection
        ...
        self.SP_Begin_Angle.valueChanged.connect(
            self.on_SP_Begin_Angle)       # connection
        ...
        self.SP_End_Angle.valueChanged.connect(
            self.on_SP_End_Angle)           # connection
        ...
        self.CB_B_E_Angle.clicked.connect(self.on_CB_B_E_Angle)
...
#        self.RA_Wire.setChecked(True)
        self.RA_Wire.setIcon(QtGui.QIcon.fromTheme(
            "Draft", QtGui.QIcon(":/icons/Draft_Wire.svg")))
        ...
        self.RA_BSpline.setIcon(QtGui.QIcon.fromTheme(
            "Draft", QtGui.QIcon(":/icons/Draft_BSpline.svg")))
        self.RA_BSpline.clicked.connect(
            self.on_RA_BSpline)  # connection radioButton
...
#        self.CH_Points.setStatusTip(_fromUtf8(""))
        ...
        self.CH_Points.setIcon(QtGui.QIcon.fromTheme(
            "Draft", QtGui.QIcon(":/icons/Draft_Point.svg")))
...
#        self.CH_Reverse.setStatusTip(_fromUtf8(""))
        ...
        self.CH_Reverse.clicked.connect(
            self.on_CH_Reverse)                    #
        ####
        ...
        self.groupBox_04.setStyleSheet(
            "background-color: QPalette.Base")      # origin system

        self.PB_Begin_End = QtWidgets.QPushButton()
        self.PB_Begin_End.clicked.connect(
            self.on_PB_Begin_End)                # connection

        self.PB_Reverse_Spr = QtWidgets.QPushButton()
        self.PB_Reverse_Spr.clicked.connect(
            self.on_PB_Reverse_Spr)            # connection
        ...
        self.DS_horizontalSlider.valueChanged.connect(
            self.on_DS_horizontalSlider)  # connection doubleSpinBox
        ...
        self.horizontalSlider.valueChanged.connect(
            self.on_horizontal_slider)  # connection
        ####
        ...
        self.SP_Numbering_Spire.valueChanged.connect(
            self.on_S_Numbering_Spire)  # connection SpinBox

        self.CH_Smooting = QtWidgets.QCheckBox()
        self.CH_Smooting.setEnabled(True)

        self.CH_Smooting.clicked.connect(
            self.on_CH_Smooting)                  # connection
        ...
        self.SP_Lissage.valueChanged.connect(
            self.on_S_Lissage)  # connection SpinBox
        ...
        self.DS_Pas_Spire.valueChanged.connect(
            self.on_DS_Pas_Spire)  # connection doubleSpinBox

        self.PU_To_Pas = QtWidgets.QPushButton()
        self.PU_To_Pas.clicked.connect(
            self.on_PU_To_Pas)                      # connection
...
#        self.DS_Radius_Spire.setMinimum(0.001)
        ...
        self.DS_Radius_Spire.valueChanged.connect(
            self.on_DS_Radius_Spire)     # connection doubleSpinBox

        self.PU_To_Radius = QtWidgets.QPushButton()
        self.PU_To_Radius.clicked.connect(
            self.on_PU_To_Radius)                # connection
...
#        section progressBar 1
        ...
        self.PU_Accept_Value.setIcon(
            QIcon(QApplication.style().standardIcon(QStyle.SP_DialogApplyButton)))
        self.PU_Accept_Value.clicked.connect(
            self.on_PU_Accept_Value)          # connection

        self.PB_Clear = QtWidgets.QPushButton()
        self.PB_Clear.setIcon(
            QIcon(QApplication.style().standardIcon(QStyle.SP_DialogResetButton)))
        self.PB_Clear.clicked.connect(
            self.on_PB_Clear)                        # connection

        self.PB_Zoom = QtWidgets.QPushButton()
        self.PB_Zoom.setIcon(
            QIcon(QApplication.style().standardIcon(QStyle.SP_FileDialogContentsView)))
        self.PB_Zoom.clicked.connect(
            self.on_PB_Zoom)                          # connection
        ...
        #
        icon02 = QtGui.QIcon()
        icon02.addPixmap(QtGui.QPixmap(self.image_02),
                         QtGui.QIcon.Normal, QtGui.QIcon.Off)  #
        #
        self.PU_Read.setIcon(icon02)
        self.PU_Read.clicked.connect(
            self.on_PU_Read)                          # connection

        self.PU_Save = QtWidgets.QPushButton()
        self.image_03 = path+"FCSpring_Helix_Variable_Icon_03.png"                          # image
        #
        icon03 = QtGui.QIcon()
        icon03.addPixmap(QtGui.QPixmap(self.image_03),
                         QtGui.QIcon.Normal, QtGui.QIcon.Off)  #
        #
        ...
        self.image_02b = path + \
            "FCSpring_Helix_Variable_Icon_02b.png"                         # image
        #
        icon02b = QtGui.QIcon()
        icon02b.addPixmap(QtGui.QPixmap(self.image_02b),
                          QtGui.QIcon.Normal, QtGui.QIcon.Off)
        #
        self.PU_Read_Coord.setIcon(icon02b)
        self.PU_Read_Coord.clicked.connect(
            self.on_PU_Read_Coord)              # connection

        self.PU_Save_Coord = QtWidgets.QPushButton()
        self.image_03b = path + \
            "FCSpring_Helix_Variable_Icon_03b.png"                         # image
        #
        icon03b = QtGui.QIcon()
        icon03b.addPixmap(QtGui.QPixmap(self.image_03b),
                          QtGui.QIcon.Normal, QtGui.QIcon.Off)
        #
        self.PU_Save_Coord.setIcon(icon03b)
        self.PU_Save_Coord.clicked.connect(
            self.on_PU_Save_Coord)              # connection
        ####
        ...
        self.CB_05_Position.clicked.connect(
            self.on_CB_05_Position)  # connection checkBox

        self.PU_Quit = QtWidgets.QPushButton()
        self.image_04 = path+"FCSpring_Helix_Variable_Icon_04.png"                          # image
        #
        icon04 = QtGui.QIcon()
        icon04.addPixmap(QtGui.QPixmap(self.image_04),
                         QtGui.QIcon.Normal, QtGui.QIcon.Off)  #
        #
        self.PU_Quit.setIcon(icon04)
        self.PU_Quit.clicked.connect(
            self.on_PU_Quit)                          # connection

        self.PU_Reset = QtWidgets.QPushButton()
        self.image_05 = path+"FCSpring_Helix_Variable_Icon_05.png"                          # image
        #
        icon05 = QtGui.QIcon()
        icon05.addPixmap(QtGui.QPixmap(self.image_05),
                         QtGui.QIcon.Normal, QtGui.QIcon.Off)  #
        #
        self.PU_Reset.setIcon(icon05)
        self.PU_Reset.clicked.connect(
            self.on_PU_Reset)                        # connection

        self.PU_Launch = QtWidgets.QPushButton()
        self.PU_Launch.setIcon(
            QIcon(QApplication.style().standardIcon(QStyle.SP_DialogOkButton)))
        self.PU_Launch.clicked.connect(
            self.on_PU_Launch)                      # connection

        self.PU_Help = QtWidgets.QPushButton()
        self.PU_Help.setIcon(QIcon(QApplication.style().standardIcon(
            QStyle.SP_MessageBoxQuestion)))  # image
        self.PU_Help.clicked.connect(
            self.on_PU_Help)                          # connection
        ...
        # englobe le cadre principal (titre + date)
        vbox.addWidget(self.groupBox0)
        ####
        ...
        if seTPositionFlyRightLeft == 1:                                                 # 1=MainWindow separate
            MainWindow.setWindowTitle(
                __Title__ + u" rmu (" + __Version__ + ") (" + __Date__ + ")")
    ...
    def retranslateUi(self, MainWindow):
        global sel
        # PySide cette fonction met la fenetre en avant
        MainWindow.setWindowFlags(PySide2.QtCore.Qt.WindowStaysOnTopHint)

        # change l'icone de la fenetre principale
        MainWindow.setWindowIcon(QtGui.QIcon(
            path+'FCSpring_Helix_Variable.png'))
        MainWindow.setWindowTitle("FCSpring Helix Variable")
####
        self.groupBox0.setTitle(
            "ver : " + __Version__ + " : " + __Date__ + " (rmu)")
        self.groupBox_01.setTitle(_fromUtf8("Configure"))
        self.DS_Numb_Spires.setToolTip(
            _fromUtf8("Number total of coil of the spring"))
        self.DS_Numb_Spires.setSuffix(" coils")
        self.LAB_Numb_Spires.setText(_fromUtf8("Number of coil"))
        self.LAB_Numb_Spires.setToolTip(
            _fromUtf8("Number total of coil of the spring"))
        self.LAB_Pas_Spring.setText(_fromUtf8("Pitch of spring"))
        self.LAB_Pas_Spring.setToolTip(
            _fromUtf8("Pitch of spring (distance between two vertices)"))
        self.DS_Pas_Spring.setToolTip(
            _fromUtf8("Pitch of spring (distance between two vertices)"))
        ...
        self.DS_Precision_Turn.setSuffix(
            "  ( " + str(int(360/precision)) + " points )")
        ...
        self.CH_Cone.setToolTip(
            _fromUtf8("Check for create one conical helix"))
...
#####
        ...
        self.CH_Points.setToolTip(
            _fromUtf8("Check to create points to alls nodes of the precision of turn"))
        self.CH_Points.setText(_fromUtf8("Points"))
        self.CH_Reverse.setToolTip(
            _fromUtf8("Check to create spring clockwise direction"))
...
#####
        self.groupBox_04.setTitle(
            _fromUtf8("Position ( " + str(len(sel)) + " )"))
        self.groupBox_04.setToolTip(
            _fromUtf8("Position (number selection) number of points"))
        self.PB_Begin_End.setToolTip(
            _fromUtf8("Align the spring to begin , end or center axis "+"\n"))
        self.PB_Begin_End.setText(_fromUtf8("Begin / End"))
        self.PB_Reverse_Spr.setText(_fromUtf8("Reverse Spr."))
        self.PB_Reverse_Spr.setToolTip(
            _fromUtf8("Reverse the spring on its axis "+"\n"))
        self.DS_horizontalSlider.setToolTip(
            _fromUtf8("Placement spring along the axis"+"\n"))
        self.DS_horizontalSlider.setSuffix(" mm")
        self.PB_Reverse_Com.setText(_fromUtf8("Reverse Count."))
        self.PB_Reverse_Com.setToolTip(
            _fromUtf8("Reverse the counter of the spring on its axis "+"\n"))
#####
        self.groupBox_05.setTitle(_fromUtf8("Coil special dimension"))
        self.groupBox_05.setToolTip(_fromUtf8(
            "This section allows you to adjust the distance from the coil named. EX: 1 spire = 2 mm"))
        self.SP_Numbering_Spire.setToolTip(_fromUtf8(
            "Numbering of coil for 1 to max = Number of coil ("+str(numberSpires)+")"))
        self.LAB_Numbering_Spire.setToolTip(
            _fromUtf8("Numbering of coil for 1 to max =  Number of coil"))
        ...
        self.CH_Smooting.setToolTip(_fromUtf8("Smoothing the line if the difference between two coils"+"\n" +
                                              ...
                                              "The finish and precision are influenced by this value"+"\n" +
                                              "PS: The result can be satisfying or completely wrong (prototype state)"))
        self.DS_Pas_Spire.setToolTip(
            _fromUtf8("Pitch of the coil (The pitch is the distance betwen 2 vertices)"))
        self.DS_Pas_Spire.setSuffix(" mm")
        self.LAB_Pas_Spire.setToolTip(
            _fromUtf8("Pitch of the coil (The pitch is the distance betwen 2 vertices)"))
        self.LAB_Pas_Spire.setText(_fromUtf8("Pitch of coil"))
        self.PU_To_Pas.setText(str(pas) + " mm")
        self.PU_To_Pas.setToolTip(
            _fromUtf8("Align the value to pitch global configured"))
        ...
        self.PU_To_Radius.setToolTip(
            _fromUtf8("Align the value to radius global configured"))
        ...
        self.PU_Accept_Value.setToolTip(
            _fromUtf8("Accept the value for the coil "))
...
####
        ...
        self.CB_05_Position.setToolTip(
            u"Change the window macro position Right of Left")
        ##
        ...
        self.PU_Launch.setToolTip(
            _fromUtf8("Launch the macro and create the spring"))
        self.PU_Help.setText(_fromUtf8("Help "))
        self.PU_Help.setToolTip(
            _fromUtf8("Dislay the wiki page in the FreeCAD browser"))
####

    # Reset 0=Reset total 1=Reset mise a jour
    def on_PU_Reset(self, zero=0):
        global numberSpires
        global rayon
        global pas
        global precision
        ...
        switchAdaptRadius = 0
        self.PB_Adapt_Radius.setEnabled(False)
        self.PB_Adapt_Radius.setStyleSheet(
            "background-color: QPalette.Base")  # origin system
        self.PB_Adapt_Radius.setText("Normal")

        self.PB_Center_Point.setEnabled(False)
        self.PB_Center_Point.setStyleSheet(
            "background-color: QPalette.Base")  # origin system
        centerFaceOrPoint = 0
        self.PB_Center_Point.setText(_fromUtf8("Point Mouse"))

        self.CB_Position.setEnabled(False)
        self.CB_Position.setStyleSheet(
            "background-color: QPalette.Base")      # origin system

        self.PB_CreaCircle.setEnabled(False)
        self.PB_CreaCircle.setStyleSheet(
            "background-color: QPalette.Base")    # origin system
        del selectedCircle[:]
...
#        App.Console.PrintMessage("on_PU_Reset "+"\n")
    ...
    # nombre de spire total
    def on_DS_Numb_Spires(self, value):
        global pas
        global numberSpires
        numberSpires = value
        a = ui
        a.on_PU_Reset(1)
        self.SP_Numbering_Spire.setToolTip(_fromUtf8(
            "Numbering of coil for 1 to max = Number of spires ("+str(numberSpires)+")"))
#        App.Console.PrintMessage("on_DS_Numb_Spires "+str(numberSpires)+"\n")

    # rayon axial du ressort
    def on_DS_Radius_Sping(self, value):
        global rayon
        rayon = value
        self.PU_To_Radius.setText(str(rayon) + " mm")
        a = ui
...
#        App.Console.PrintMessage("on_DS_Radius_Sping "+str(rayon)+"\n")
    ...
    # pas (pitch) du ressort
    def on_DS_Pas_Spring(self, value):
        global pas
        global numberSpires
        global numberSpiresModified
        global pasSpire
...
#        App.Console.PrintMessage("on_DS_Pas_Spring "+str(pas)+"\n")
    ...
    # " ! float " 360/precision number points for 1 turn
    def on_DS_Precision_Turn(self, value):
        global precision
        precision = value
        # font and color Label
        font = QtGui.QFont()
...
#        App.Console.PrintMessage("on_CH_Points "+"\n")
    ...
    # reverse the rotation Rigth/Left
...
#        App.Console.PrintMessage("on_CH_Reverse " + str(spireReverse)+"\n")
    ...
    # Diametre num 2 , Helix Conique
    def on_DS_Radius_2_Cone(self, value):
        global rayon
        global numberSpires
        global helixS
        global radiusS
...
#        App.Console.PrintMessage("on_CB_Adapt_Radius "+str(switchAdaptRadius)+"\n")
    ...
    # Center Face or Point
    def on_PB_Center_Point(self):
        global centerFaceOrPoint
...
#        App.Console.PrintMessage("on_CB_Center_Point "+str(centerFaceOrPoint)+"\n")
    ...
    # place the spring to left center rigth
    def on_PB_Begin_End(self):
        global Direction_Begin
        global pointsDirection
        global ressort
        global coor_Z
        ...
        if (Direction_Begin == 0):
            ...
            ressort.Placement.Base = pointsDirection[int(
                (len(pointsDirection) / 2))]
...
#        App.Console.PrintMessage("on_CB_01_Begin_End"+"\n") FreeCAD.Vector(
    ...
    # reverse the spring
    def on_PB_Reverse_Spr(self):
        global ressort
        a = FreeCAD.ActiveDocument.getObject(ressort.Name).Placement
        b = App.Placement(App.Vector(0.0, 0.0, 0.0), App.Rotation(
            App.Vector(0.0, 1.0, 0.0), 180.0), App.Vector(0, 0, 0))
        c = a.multiply(b)
...
#        App.Console.PrintMessage("on_PB_Reverse_Spr"+"\n")
    ...
    def on_DS_horizontalSlider(self, val):                                      #
        ...
        if len(pointsDirection) != 0:
            ...
            try:
                ressort.Placement.Base = FreeCAD.Vector(
                    pointsDirection[int(val*10)])
            except Exception:
                None
            FreeCAD.ActiveDocument.recompute()
#        App.Console.PrintMessage("on_DS_horizontalSlider"+"\n")

    # reverse the compteur
    def on_PB_Reverse_Com(self):
        global pointsDirection
        global ressort
...
#        App.Console.PrintMessage("on_PB_Reverse_Com"+"\n")
    ...
    # connection on_horizontal_slider
...
#        App.Console.PrintMessage("on_horizontal_slider " + str(val/10)+"\n")
    ...
    def on_CB_Position(self):
        ...
        if self.CB_Position.isChecked():
            ...
            try:
                Direction_Begin = 0
                pointsDirection = sel[0].Object.Shape.discretize(
                ...
                self.groupBox_04.setTitle(
                    "Position (" + str(len(sel)) + ")(Points: " + str(len(pointsDirection)) + ")")
                self.horizontalSlider.setMaximum(len(pointsDirection))
                self.DS_horizontalSlider.setMaximum(
                    int(len(pointsDirection)/10))
...
#        App.Console.PrintMessage("on_CB_Position "+"\n")
    ...
    # create circle on 3 points
    def on_PB_CreaCircle(self):
        global selectedCircle
        ...
        if len(selectedCircle) == 3:
            if (selectedCircle[0] == selectedCircle[1]) or (selectedCircle[0] == selectedCircle[2]) or selectedCircle[1] == selectedCircle[2]:
                App.Console.PrintError(
                    "Bad selection the points are egual"+"\n")
            else:
                try:
                    C1 = Part.Arc(FreeCAD.Vector(selectedCircle[0]), FreeCAD.Vector(
                        selectedCircle[1]), FreeCAD.Vector(selectedCircle[2]))
                    # create arc base
                    S1 = Part.Shape([C1])
                    Part.show(S1)

                    # select the object created
                    ...
                    App.ActiveDocument.removeObject(
                        obj.Label)                 # remove arc master

                    # give direction to circle
                    v = CircleDirection
                    r = App.Rotation(App.Vector(0, 0, 1), v)
                    pl = FreeCAD.Placement()
                    pl.Base = FreeCAD.Vector(CircleAxis)
                    pl.Rotation.Q = r.Q
                    circle3Points = Draft.makeCircle(
                        radius=CircleRayon, placement=pl, face=False, support=None)
                    circle3Points.ViewObject.LineColor = (
                        1.0, 0.0, 0.0)         # give color
                    # give label
                    circle3Points.Label = "SpringCircle (" + \
                        str(round(CircleRayon, 3)) + " r)"
                except Exception:
                    App.Console.PrintError(
                        "Three points are collinear or bad selection"+"\n")
            del selectedCircle[:]
            FreeCAD.ActiveDocument.recompute()
        # self.PB_CreaCircle.setEnabled(False)
#        App.Console.PrintMessage("on_PB_CreaCircle "+"\n")
    ...
    # numero de la spire a modifier
    def on_S_Numbering_Spire(self, value):
        global numberSpires
        global numberSpiresModified
        global lissageSpire
...
#        App.Console.PrintMessage("on_CH_Smooting "+str(lissageSpire)+"\n")
    ...
    # option lissageS des raccords
    def on_S_Lissage(self, value):
        global lissageSpire
        global precision
...
#        App.Console.PrintMessage("on_S_Lissage "+str(lissageSpire)+"\n")
    ...
    # pas (pitch) de la spire a modifier
    def on_DS_Pas_Spire(self, value):
        global pasSpire

        pasSpire = value
#        App.Console.PrintMessage("on_DS_Pas_Spire "+str(pasSpire)+"\n")

    # aligne le pas (pitch) de la spire
    def on_PU_To_Pas(self):
        global pasSpire
        global pas
...
#        App.Console.PrintMessage("on_PU_To_Pas "+str(pasSpire)+"\n")
    ...
    # pas (pitch) du rayon a modifier
    def on_DS_Radius_Spire(self, value):
        global rayonSpire
        rayonSpire = value
#        App.Console.PrintMessage("on_DS_Radius_Spire "+str(rayonSpire)+"\n")

    # aligne le rayon
    def on_PU_To_Radius(self):
        global rayonSpire
        global rayon
...
#        App.Console.PrintMessage("on_PU_To_Radius_Value "+str(rayonSpire)+"\n")
    ...
    # accepter la modification de la spire
    def on_PU_Accept_Value(self):
        global numberSpiresModified
        global numberSpires
        global pasSpire
        global rayonSpire
        ...
        if rayonSpire != 0:
            if numberSpiresModified == 1:
                ...
                self.textEdit.append("Coil number " + str(numberSpiresModified) + " = " + str(
                    helixS[0]) + " " + str(radiusS[0]) + " " + str(lissageS[0]))
                App.Console.PrintMessage("Coil number " + str(numberSpiresModified) + " = " + str(
                    helixS[0]) + " " + str(radiusS[0]) + " " + str(lissageS[0])+"\n")
            else:
                ...
                self.textEdit.append("Coil number " + str(numberSpiresModified) + " = " + str(helixS[numberSpiresModified-1]) + " " + str(
                    radiusS[numberSpiresModified-1]) + " " + str(lissageS[numberSpiresModified-1]))
                App.Console.PrintMessage("Coil number " + str(numberSpiresModified) + " = " + str(helixS[numberSpiresModified-1]) + " " + str(
                    radiusS[numberSpiresModified-1]) + " " + str(lissageS[numberSpiresModified-1])+"\n")
            ...
            self.textEdit.append(
                "Coil number " + str(numberSpiresModified) + " Radius zero not allowed do ")
...
#        App.Console.PrintMessage("on_PB_Clear "+"\n")
    ...
    def on_PB_Zoom(self):                                                     #
        ...
        if zoom == 0:
            ...
            zoom = 0
            self.grid_05.addWidget(self.PU_Accept_Value, 3, 0, 1, 4)
            self.PU_Accept_Value.setText(
                _fromUtf8("Accept the value modified"))
            self.grid_05.addWidget(self.textEdit, 5, 0, 2, 3)
#        App.Console.PrintMessage("on_PB_Loupe "+"\n")
    ...
    def on_PU_Quit(self):                                                      # Quit
        ...
        # desinstalle la fonction residente
        FreeCADGui.Selection.removeObserver(s)
        try:
            self.window.setAttribute(
                QtCore.Qt.WA_DeleteOnClose, True)    # destroy
...
#        App.Console.PrintMessage(" "+"\n")
    ...
    def on_PU_Read(self):                                                      # lecture
        ...
        if switchQFileDialogMint == True:   # Mint
            OpenName, Filter = PySide2.QtWidgets.QFileDialog.getOpenFileName(
                None, u"Read a file FCSpring", setPathLatestDirectory, "(FCSpring *.FCSpring);;")  # PySide2 Mint
        # mint
        else:
            OpenName, Filter = PySide2.QtWidgets.QFileDialog.getOpenFileName(
                None, u"Read a file FCSpring", setPathLatestDirectory, "*.FCSpring;;")  # PySide2

        try:
            if OpenName != "":
                # new2
                # 1# = C:/Provisoire400/
                pathFile = os.path.dirname(OpenName) + "/"
                setPathLatestDirectory = pathFile
                FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" + __Title__).SetString(
                    u"setPathLatestDirectory", setPathLatestDirectory)  # *"C:\ ???"
                    ...
                    if (Header == "FCString") or (Header == "FCString2") or (Header == "FCString3") or (Header == "FCString4"):
                        ...
                        if (Header == "FCString2") or (Header == "FCString3") or (Header == "FCString4"):
                            ...
                            if (spireConeUne == 1) or (spireConeComp == 1):
                                ...
                                self.LAB_Numb_Spires.setText(
                                    "Number real ("+str(numberSpires - 1)+")")
...
#                                self.CH_Cone.setEnabled(False)
                                self.LAB_Numb_Spires.setText("Number of coil")
                                self.LAB_Numb_Spires.setToolTip(
                                    _fromUtf8("Number of coil"))
                            App.Console.PrintMessage(
                                str(spireConeUne)+" "+str(radius_2_Cone)+"\t"+"Radius cone"+"\n\n")
                        ...
                        for ligne in file:
                            if (Header == "FCString4"):
                                ...
                                lissageS[i] = int(c)
                                App.Console.PrintMessage(
                                    str(i+1)+":  "+str(helixS[i])+"   "+str(radiusS[i])+"   "+str(lissageS[i])+"\n")
                                self.textEdit.append("Coil number " + str(i+1) + " = " + str(
                                    helixS[i]) + " " + str(radiusS[i])+"   "+str(lissageS[i]))  # 12
                            else:
                                a, b = ligne.rstrip('\n\r').split()
                                helixS[i] = float(a)
                                radiusS[i] = float(b)
                                App.Console.PrintMessage(
                                    str(i+1)+":  "+str(helixS[i])+"   "+str(radiusS[i])+"\n")
                                self.textEdit.append("Coil number " + str(i+1) + " = " + str(
                                    helixS[i]) + " " + str(radiusS[i]))                        # 12
                            i += 1
                        App.Console.PrintMessage("____________________"+"\n")
                        ...
                        App.Console.PrintMessage(
                            "Error file not FCSpring or not FCSpring2.3.4"+"\n")
                        errorDialog(
                            "Error file not FCSpring or not FCSpring2.3.4 "+"\n")
                finally:
                    file.close()
            ...
            App.Console.PrintMessage(
                "Error in reading the file "+OpenName+"\n")
    ...
    def on_PU_Save(self):
        ...
        # mint
        if switchQFileDialogMint == True:   # Mint
            SaveName, Filter = PySide2.QtWidgets.QFileDialog.getSaveFileName(
                None, "Save a file FCSpring", setPathLatestDirectory, " (FCSpring *.FCSpring);;")  # PySide2 Mint
            Filter = Filter[Filter.find("."):Filter.find(")")]
            SaveName = SaveName + Filter
        # mint
        else:
            SaveName, Filter = PySide2.QtWidgets.QFileDialog.getSaveFileName(
                None, "Save a file FCSpring", setPathLatestDirectory, "*.FCSpring;;FCSpring (*.FCSpring);;")  # PySide2
        ...
        if SaveName == "":
            ...
            # new2
            pathFile = os.path.dirname(SaveName) + \
                "/"  # 1# = C:/Provisoire400/
            setPathLatestDirectory = pathFile
            FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                             __Title__).SetString(u"setPathLatestDirectory", setPathLatestDirectory)  # *"C:\ ???"
            ...
            try:
                ...
                try:
                    ...
                    file.write(str(spireConeUne)+"\t"+str(spireConeComp) +
                               "\t"+str(radius_2_Cone)+"\n")             # 10
                    file.write("Coil"+"\t"+"Radius"+"\t"+"Smooting"+"\t" +
                               "(If you change the file, always use Tab no space)"+"\n")  # 11
                    for i in range(numberSpires):
                        # 12
                        file.write(
                            str(helixS[i])+"\t"+str(radiusS[i])+"\t"+str(lissageS[i])+"\n")
                finally:
                    file.close()
            except Exception:
                App.Console.PrintError(
                    "Error Registration file "+SaveName+"\n")
                errorDialog("Error Registration file "+SaveName)

    # Save coordinates x y z
    def on_PU_Save_Coord(self):
        global points
        global setPathLatestDirectory
        ...
        if len(points) > 2:
            ...
            if switchQFileDialogMint == True:   # Mint
                SaveName, Filter = PySide2.QtWidgets.QFileDialog.getSaveFileName(
                    None, "Save a file FCSpringCoor", setPathLatestDirectory, "(FCSpringCoor *.FCSpringCoor);;)")  # PySide2 Mint
                Filter = Filter[Filter.find("."):Filter.find(")")]
                SaveName = SaveName + Filter
            # mint
            else:
                SaveName, Filter = PySide2.QtWidgets.QFileDialog.getSaveFileName(
                    None, "Save a file FCSpringCoor", setPathLatestDirectory, "*.FCSpringCoor;;FCSpringCoor (*.FCSpringCoor);;")  # PySide2
...
#        App.Console.PrintMessage("on_PU_Save_Coord"+"\n")
    ...
    def on_PU_Read_Coord(self):
        ...
        if switchQFileDialogMint == True:   # Mint
            OpenName, Filter = PySide2.QtWidgets.QFileDialog.getOpenFileName(
                None, u"Read a file FCSpringCoor", setPathLatestDirectory, "(FCSpringCoor *.FCSpringCoor);;")  # PySide2 Mint
        # mint
        else:
            OpenName, Filter = PySide2.QtWidgets.QFileDialog.getOpenFileName(
                None, u"Read a file FCSpringCoor", setPathLatestDirectory, "*.FCSpringCoor")  # PySide2
...
#                    self.PBA_progressBar.setValue(0)
                    ...
                    if typeLine == 1:
                        # creation spring makeWire
                        ressort = Draft.makeWire(
                            points, closed=False, face=False, support=None)
                    else:
                        # creation spring Draft " makeBSpline " remis pour facilite avec axes
                        ressort = Draft.makeBSpline(points, closed=False)
...
#                        Part.show(ressort0)
                    ...
                    try:
                        App.ActiveDocument.ActiveObject.Label = "Spring_" + \
                            UNICODE(nomF)
                    except Exception:
                        try:
                ...
                self.textEdit.append(
                    "PS: For Info The values showing in the configuration menu do not match the data of the spring"+"\n")
                self.textEdit.setTextColor(QColor("Base"))
        ...
        ############ Section object selected Begin ####################################################
            ...
            try:
                if len(sel) != 0:
                    ...
                    FCSpring = doc.addObject(
                        "App::DocumentObjectGroup", "FCSpring")
                    switchReverse = 0
                    xL = yL = 0.0
                    # calcul hauteur Cylindre / axe
                    hauteurCylindre = sum(helixS)
                    ...
                    if hasattr(sel[0].SubObjects[0], "Surface"):
                        ...
                        if (str(subObjet.Surface) == "<Cylinder object>") or (str(subObjet.Surface) == "<SurfaceOfExtrusion object>"):      # face
                            try:
                                ...
                                App.Console.PrintError(
                                    "<Cylinder object> + <SurfaceOfExtrusion object"+"\n")
                            ...
                            try:
                                ...
                                if centerFaceOrPoint != 0:
                                    # Center Of face
                                    yL = sel[0].SubObjects[0].CenterOfMass
                                else:
                                    yL = vecteurSouris                         # clic point
                                uv = comp.Surface.parameter((yL))
                                nv = comp.normalAt(uv[0], uv[1]).normalize().multiply(
                                    hauteurCylindre)
                                xL = nv + yL
                                points = [xL, yL]
                            except Exception:
                        ...
                        # Sphere
                        elif (str(subObjet.Surface)[0:6] == "Sphere"):
                            try:
                                #                                print( "Sphere" )
                                subObjet = sel[0].SubObjects[0]
                                if switchAdaptRadius == 1:
                                    # selobject[0].Radius2.Value # option
                                    rayon = selobject[0].Radius.Value
                                    self.DS_Radius_Sping.setValue(rayon)
                        ...
                        # Toroid + Cone
                        elif (str(subObjet.Surface) == "<Toroid object>") or (str(subObjet.Surface) == "<Cone object>"):
                            try:
                                #                                print( str(subObjet.Surface) )
                                subObjet = sel[0].SubObjects[0]
                                if switchAdaptRadius == 1:
                                    # selobject[0].Radius2.Value # option
                                    rayon = selobject[0].Radius1.Value
                                    self.DS_Radius_Sping.setValue(rayon)
                                obj = selobject[0]
                                comp = subObjet
                                if centerFaceOrPoint != 0:
                            ...
                            try:
                                ...
                                uv = comp.Surface.parameter((xL))
                                nv = comp.normalAt(uv[0], uv[1]).normalize().multiply(
                                    hauteurCylindre)  # hauteurCylindre
                                yL = nv + xL
                                points = [xL, yL]
                            except Exception:
                        ...
                        if (len(sel[0].SubObjects) == 2):
                            ...
                            try:
                                # arc or circle
                                if (hasattr(subObjet.Curve, "Center")) and (hasattr(subObjet.Curve, "Radius")):
                                    #                                    print( "Circle/Arc" )
                                    subObjet = sel[0].SubObjects[0]
                                    if switchAdaptRadius == 1:
                                    ...
                                    direction = subObjet.Curve.Axis.normalize().multiply(
                                        hauteurCylindre)                      # option
                                    # edge
                                    edgeO = selobject[0]
                                    ...
                                    xL = sel[0].SubObjects[0].Vertexes[0].Point
                                    yL = xL + \
                                        (FreeCAD.Vector(0.0, 0.0, hauteurCylindre))
                                    points = [xL, yL]
                            except Exception:
                                xL = sel[0].SubObjects[0].Vertexes[0].Point
                                try:
                    ...
                    axisSpring = sel = ""
                    axisSpring = Draft.makeWire(
                        points, closed=False, face=False, support=None)  # make line
                    # give axis LineColor
                    axisSpring.ViewObject.LineColor = (1.0, 0.0, 0.0)
                    axisSpring.ViewObject.DrawStyle = "Dashdot"
                    axisSpring.Label = "Spring Axis (" + \
                        str(axisSpring.Length.Value) + ")"
                    ...
                    sel = FreeCADGui.Selection.getSelectionEx()
                    pointsDirection = sel[0].Object.Shape.discretize(
                        Distance=Direction_Distance)  # discretize
...
#                self.groupBox_04.setTitle("Position (" + str(len(sel)) + ")(Points: " + str(len(pointsDirection)) + ")")
        ...
        ############ Section object selected End ######################################################
            ...
            self.PU_Accept_Value.setVisible(False)
            self.PBA_progressBar.setVisible(True)
            self.PBA_progressBar.setMaximum(
                numberSpires * (360 / int(precision)))
            self.PBA_progressBar.setValue(0)
            ...
            # number spires Helix
            for spir in range(numberSpires-spireConeComp):
                pas2 = helixS[spir]
                PBA += 1

                if spir != numberSpires - 1:                                   # pas travail normal
                    pastr = (
                        (radiusS[spir + 1]-radiusS[spir]) / (360/precision))
                ...
                # or (spir == numberSpires):
                if (spir == numberSpires - 1 - spireConeComp):
                    finAngleX = finAngle + int(precision)

                # boucle for 1 turn (360/precision) degrees
                for angleTr in range(0, finAngleX, int(precision)):
                    pasRadius = radiusS[spir]+((pastr*i))
                    i += 1
                    self.PBA_progressBar.setValue(PBA)
                    PBA += 1

                    vecligne = [FreeCAD.Vector(
                        0, 0, 0), FreeCAD.Vector(pasRadius, 0.0, 0.0)]
                    # creation de la ligne de base
                    ligne = Draft.makeWire(
                        vecligne, closed=False, face=False, support=None)
                    #ligneName = ligne.Name

                    if spireReverse == 0:                                      # counterclockwise
                        ligne.Placement = FreeCAD.ActiveDocument.Line.Placement = App.Placement(App.Vector(
                            0.0, 0.0, 0.0), App.Rotation(App.Vector(0, 0, 1), angleTr), App.Vector(0, 0, 0))
                    ...
                    try:
                        # fin de ligne
                        a = ligne.Shape.Edges[0].Vertexes[1]
                    except Exception:
                    ...
                    if angleTr >= debutAngleX:
                        # coordinates makeBSpline contener
                        points += [FreeCAD.Vector(coor_X, coor_Y, coor_Z)]

                    if (affPoint == 1) and (angleTr >= debutAngleX):
                        # create point repere for test
                        point = Draft.makePoint(coor_X, coor_Y, coor_Z)
                        FreeCADGui.activeDocument().getObject(point.Name).PointColor = (1.0, 0.0, 0.0)

                    # pas of spring
                    coor_Z += (pas2 / (360/precision))

                    # remove ligne de base directrice
            ...
            if (numberSpires >= 2) and (sum(lissageS[:]) != 0):
                ...
                # decalage montant
                decalageM = 0
                # nombre de points dans la boucle
                pointTravailBase = int(360/precision)

                if debutAngle != 0:
                    # decalageM debutAngle
                    decalageM = (pointTravailBase -
                                 int((360-debutAngle)/precision))
                    # lissage spire 1
                    if lissageS[0] != 0:
                ...
                if finAngle != 360:
                    # lissage derniere spire
                    if lissageS[-1] != 0:
                        if lissageS[-1] > (pointTravailBase - int((360-finAngle)/precision)):
                            lissageS[-1] = (pointTravailBase -
                                            int((360-finAngle)/precision)) - 1

                    # lissage avant derniere spire
                    if lissageS[-2] != 0:
                        if lissageS[-2] > (pointTravailBase - int((360-finAngle)/precision)):
                            lissageS[-2] = (pointTravailBase -
                                            int((360-finAngle)/precision)) - 1

                    try:
                        # lissage avant derniere spire cas cone (N_spire-1)
                        if (lissageS[-3] != 0) and ((spireConeUne == 1) or (spireConeComp == 1)):
                            if lissageS[-3] > (pointTravailBase - int((360-finAngle)/precision)):
                                lissageS[-3] = (pointTravailBase -
                                                int((360-finAngle)/precision)) - 1
                    except Exception:
                        None
                ...
                for ii in range(1, numberSpires):
                    ...
                    self.PBA_progressBar.setValue(
                        compBar1)                    # progressBar
                    ...
                    if lissageS[ii-1] != 0:
                        ...
                        if (ii-1 == 0) and (debutAngle != 0):                  # nombre de points
                            if (boucler) > (pointTravailBase - int((360-debutAngle)/precision)):
                                boucler = lissageS[0] - (pointTravailBase -
                                                         int((360-debutAngle)/precision))
                        ...
                        try:
                            for i in (range(boucler)):
                                ...
                                if i == 0:
                                    # G          # C
                                    a = points[pointTravail-1-decalageM][2]
                                    # D          # C
                                    b = points[pointTravail+1-decalageM][2]
                                    # C
                                    c = (((b - a)/x0) + a)
                                    # C
                                    points[pointTravail+(i)-decalageM][2] = c
                                else:
                                    x1 += 1
                                    # G    # D
                                    a = points[pointTravail +
                                               (x1-x2)-decalageM][2]
                                    # D    # D
                                    b = points[pointTravail +
                                               (x1+x2)-decalageM][2]
                                    # D
                                    c = (((b - a)/x3) + a)
                                    # D
                                    points[pointTravail+(x1)-decalageM][2] = c

                                    a = points[pointTravail +
                                               (-x1-x2)-decalageM][2]  # G   # G
                                    b = points[pointTravail +
                                               (-x1+x2)-decalageM][2]  # D   # G
                                    # G
                                    c = (((b - a)/x3) + a)
                                    # G
                                    points[pointTravail+(-x1)-decalageM][2] = c
                        ...
                        try:
                            for i in (range(boucler)):
                                ...
                                if i == 0:
                                    ...
                                    x1 += 1
                                    a = points[pointTravail +
                                               (x1-x2)-decalageM][2]  # G   # D
                                    b = points[pointTravail +
                                               (x1+x2)-decalageM][2]  # D   # D
                                    # D
                                    c = (((a - b)/x3) + b)
                                    # D
                                    points[pointTravail+(x1)-decalageM][2] = c
...
#                Part.show(ressort)                      ##
            ...
            # placement if axis selected
            if (plr != "xx") and (len(pointsDirection) != 0):
                pointsDirection.reverse()
            ...
            if nomF != "Name File":  # name file
                ...
                App.ActiveDocument.ActiveObject.Label = "Spring (" + str(
                    rayon) + " r)"
...
####on_PU_Launch end ###################################################################################
    ...
    def selectOk(self, val):
        ...
        try:
            # display the type line selected
            sel = FreeCADGui.Selection.getSelectionEx()
        ...
        if val == 1:
            ...
            self.PB_CreaCircle.setStyleSheet(
                "background-color: QPalette.Base")        # origin system
        switchAdaptRadius = 0
        # (Points: " + str(len(pointsDirection)) + ")
        self.groupBox_04.setTitle("Position (" + str(len(sel)) + ")")
    ...
    def on_CB_05_Position(self):
        ...
        if seTPositionFlyRightLeft != 1:
            if self.CB_05_Position.isChecked():
                # add the widget to the main window Left
                FCmw.addDockWidget(
                    QtCore.Qt.LeftDockWidgetArea, myNewFreeCADWidget)
                FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                                 __Title__).SetInt(u"seTPositionFlyRightLeft", 3)  # *1, 2, other
                FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                                 __Title__).SetBool(u"seTWidgetPosition", True)       # True or False
            else:
                # add the widget to the main window Right
                FCmw.addDockWidget(
                    QtCore.Qt.RightDockWidgetArea, myNewFreeCADWidget)
                FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                                 __Title__).SetInt(u"seTPositionFlyRightLeft", 2)  # *1, 2, other
                FreeCAD.ParamGet(u"User parameter:BaseApp/Preferences/Macros/FCMmacros/" +
                                 __Title__).SetBool(u"seTWidgetPosition", False)      # True or False
...
#        App.Console.PrintMessage("addSelection"+"\n")
    ...
    # Si clic sur l'ecran, effacer la selection
...
s = SelObserver()
# installe the function in resident mode
...
if i.objectName() != __Title__:                              # macro internal Name
    ...
    #
        ...
        if seTPositionFlyRightLeft == 2:             # RightDock
            ...
            # add the widget to the main window Left
            FCmw.addDockWidget(QtCore.Qt.LeftDockWidgetArea,
                               myNewFreeCADWidget)
