...
def criardocs(alimentarbd, caminhodados, arquivo):    
    ...
    def desenharreator(path, arquivo):
        ...
        app_freecad = QApplication.instance()
        if app_freecad is None:
            app_freecad = QApplication(sys.argv)    
          
        FreeCADGui.showMainWindow()       

        

        #app = FreeCAD.newDocument()  
        ...
        FreeCADGui.showMainWindow()  
        FreeCADGui.getMainWindow().setAttribute(QtCore.Qt.WA_DeleteOnClose, False)
    ...
    if(prosseguir == 1):  
        ...
        desenharreator(path + str(pi) + '-' + str(cliente) +  "/Item " + item + '/Desenhos/', arquivo)
